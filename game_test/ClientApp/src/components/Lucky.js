...
export class Lucky extends Component {
    ...
    constructor(props) {
        ...
        this.tryToGuess = this.tryToGuess.bind(this, Lucky);
        this.populateRange = this.populateRange.bind(this, Lucky);
    ...
    tryToGuess() {
        if (this.state.choice == this.state.realValue) {
            this.setScore()
            this.setState({
                reactionImage: Lucky.assets.wellDone
            });
            setTimeout(() => {
                this.setState({
                    reactionImage: null
            ...
            if (this.state.live == 1) {
                this.setState({
                    reactionImage: Lucky.assets.gameOver
                });
                this.killer();
                this.readyToPlay();
                setTimeout(() => {
                ...
                this.killer();
                this.setState({
                    reactionImage: Lucky.assets.tryAgain
                });
                setTimeout(() => {
                    this.setState({
                        reactionImage: null
        ...
        return (
            <>
                ...
                <div style={{
                    ...
                    {this.state.ready ? (
                        <button className="btn btn-primary" onClick={this.getRealNumber} style={{
                            marginBottom: '1rem'
                        }} >Let's go</button>
                    ) : null}
